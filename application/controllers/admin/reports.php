        ...
        foreach($rows->result() as $r){
            ...
            if($pdf == 'print' || $pdf == 'pdf'){
                ...
                $this->table->add_row(
                    ...
                    $this->hide_trx($r->merchant_trans_id).$fcode,
                    '',
                    ''
                );
	...
	public function devicerecon($type = null,$status = null,$year = null, $scope = null, $par1 = null, $par2 = null, $par3 = null){
        ...
        if($status != 'all'){
            ...
            $this->db->group_start();
                ...
                $this->db->or_where('status',$this->config->item('trans_status_mobile_noshow'));
                $this->db->or_where('status',$this->config->item('trans_status_mobile_rescheduled'));
                $this->db->or_where('status',$this->config->item('trans_status_mobile_return'));
                $this->db->or_();
                $this->db->group_start();
                    $this->db->where('status',$this->config->item('trans_status_new'));
                    $this->db->where('pending_count >', 0);
                $this->db->group_end();
            $this->db->group_end();
        ...
        */
		...
		foreach ($tarray as $r) {
            ...
            $ns = (isset($r['noshow']))?$r['noshow']:0;
			$rs = (isset($r['rescheduled']))?$r['rescheduled']:0;
            $ps = (isset($r['pending']))?$r['pending']:0;
            $tr = (isset($r['returned']))?$r['returned']:0;
		...
		$this->table->add_row(
			...
			array('data'=>$tdl,'style'=>'border-top:thin solid grey'),
			array('data'=>$tp,'style'=>'border-top:thin solid grey'),
            array('data'=>$tret,'style'=>'border-top:thin solid grey'),
            array('data'=>($tns + $trs),'style'=>'border-top:thin solid grey'),
			array('data'=>$gt,'style'=>'border-top:thin solid grey')
		);
